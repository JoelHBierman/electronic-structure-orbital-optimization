...
I = SparsePauliOp.from_list([("I", 1)])  # pylint: disable=invalid-name
X = SparsePauliOp.from_list([("X", 1)])  # pylint: disable=invalid-name
Z = SparsePauliOp.from_list([("Z", 1)])  # pylint: disable=invalid-name
...
class TestMCVQE(QiskitAlgorithmsTestCase):
    ...
    def test_missing_varform_params(self, op):
        ...
        ssvqe = MCVQE(estimator=self.estimator, ansatz=circuit, optimizer=SLSQP(), k=1)
        with self.assertRaises(AlgorithmError):
    ...
    def test_aux_operators_dict(self, op):
        ...
        wavefunction = self.ry_wavefunction
        ssvqe = MCVQE(estimator=self.estimator, ansatz=wavefunction, optimizer=SLSQP())
