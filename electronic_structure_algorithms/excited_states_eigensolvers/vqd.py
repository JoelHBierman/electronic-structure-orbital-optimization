...
See https://arxiv.org/abs/1805.08138. This implementation is derived from Qiskit's
implementation of VQD and has been modified to be able to handle the use
of different ansatzes for each excited state.
"""
...
class VQD(VariationalAlgorithm, Eigensolver):
    ...
    state (a.k.a. ansatz) which is a :class:`QuantumCircuit` or a list of
    :class:`QuantumCircuit`, and the classical :mod:`~qiskit.algorithms.optimizers`.
    ...
    def compute_eigenvalues(
        ...
        if isinstance(self.ansatz, QuantumCircuit):
            self.ansatz = [self.ansatz.copy()]*self.k
        elif len(self.ansatz) != self.k:
            raise AlgorithmError("The number of ansatz circuits provided does not match k, the number of states.")
        # this sets the size of the ansatz, so it must be called before the initial point
        # validation
        self._check_operator_ansatz(operator)

        bounds = [validate_bounds(ansatz) for ansatz in self.ansatz]
        ...
        if self.initial_point is not None:
            if isinstance(self.initial_point[0], np.ndarray) or isinstance(self.initial_point[0], list):
                initial_points = self.initial_point
            else:
                initial_points = [self.initial_point]
        ...
        #TODO: determine how to validate initial point for case where self.ansatz
        # is a list, but num_initial_points <= 1.
        if num_initial_points <= 1:
            initial_point = validate_initial_point(self.initial_point, self.ansatz)

        for step in range(1, self.k + 1):
            if num_initial_points > 1:
                initial_point = validate_initial_point(initial_points[step - 1], self.ansatz[step - 1])

            if step > 1:
                prev_states.append(self.ansatz[step - 2].assign_parameters(result.optimal_points[-1]))
            ...
            if callable(optimizer):
                opt_result = optimizer(  # pylint: disable=not-callable
                    fun=energy_evaluation, x0=initial_point, bounds=bounds[step - 1]
                )
            else:
            ...
            self._update_vqd_result(result, opt_result, eval_time, self.ansatz[step - 1].copy())

            if aux_operators is not None:
                aux_value = estimate_observables(
                    self.estimator, self.ansatz[step - 1], aux_operators, result.optimal_points[-1]
                )
    ...
    def _get_evaluate_energy(
        ...
        num_parameters = self.ansatz[step - 1].num_parameters
        if num_parameters == 0:
        ...
        def evaluate_energy(parameters: np.ndarray) -> np.ndarray | float:
            ...
            try:
                estimator_job = self.estimator.run(
                    circuits=[self.ansatz[step - 1]], observables=[operator], parameter_values=[parameters]
                )
            ...
            if self.callback is not None:
                ...
                for params, value, meta in zip([parameters], values, metadata):
                    self._eval_count += 1
                    self.callback(self._eval_count, params, value, meta, step, costs)
            else:
