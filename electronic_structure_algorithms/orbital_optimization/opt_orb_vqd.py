...
class OptOrbVQD(OptOrbEigensolver):
    ...
    def __init__(self,
        ...
        if excited_states_solver.__class__.__name__ != 'VQD':

            raise AlgorithmError(f"The ground state solver needs to be of type VQD, not {excited_states_solver.__class__.__name__}")
    ...
    def parameter_update_rule(self, result: OptOrbEigensolverResult,
        ...
        if self.eigensolver_random_perturbation == None or self.eigensolver_random_perturbation == 0.0:
            ...
            for n in range(self._excited_states_solver_list[iteration].k):
                self._excited_states_solver_list[iteration].initial_point[n] = result.optimal_points[n]
        else:
            
            for n in range(self._excited_states_solver_list[iteration].k):
                self._excited_states_solver_list[iteration].initial_point[n] = result.optimal_points[n] + np.random.normal(loc=0.0,
                                                scale=self.eigensolver_random_perturbation, size=result.optimal_points[n].size)

class OptOrbVQDResult(OptOrbEigensolverResult):
