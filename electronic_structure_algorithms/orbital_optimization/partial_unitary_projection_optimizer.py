...
class PartialUnitaryProjectionOptimizer():
    ...
    r"""An implementation a gradient projection optimizer with alternating Barzilai-Borwein stepsize.
        See https://epubs.siam.org/doi/10.1137/16M1098759 for reference. This algorithm optimizes an objective
        function over the space of MxN partial unitary matrices. In this instance we are interested in minimizing
        functions involving quantities measured on a quantum computer relevant to the electronic structure problem.
        e.g. the expectation values of observables."""
    ...
    def __init__(self,
        ...
        """
            Args:
                ...
                callback: A callback function to track the progress of the optimization. This callback takes the iteration number and
                    the value of the objective function as arguments.
    ...
    def compute_optimal_rotation(self, fun: Callable,
        ...
        # calculates f(U_0) and stores it in self._P4_array[2]
        P4_array[2] = objfunc(partial_unitary=current_partial_unitary)
        if self._callback is not None:
            self._callback(iteration_number, P4_array[2].item())

        # computes gradf(U_0) and stores it in self._current_rotated_energy_gradient
        current_rotated_energy_gradient = gradient_function(current_partial_unitary, objfunc)

        # calculates U_1 (from U_0 and gradf(U_0)) and stores it in temporary variable
        new_partial_unitary = self.compute_updated_partial_unitary(iteration_number=iteration_number,
                                                                   ...
                                                                   previous_rotated_energy_gradient=previous_rotated_energy_gradient)
        
        # calculates gradf(U_1)
        new_rotated_energy_gradient = gradient_function(new_partial_unitary, objfunc)

        # copies U_0 into self._previous_partial_unitary for k=1 iteration
        previous_partial_unitary = current_partial_unitary

        # copies gradf(U_0) into self._previous_rotated_energy_gradient for k=1 iteration
        previous_rotated_energy_gradient = current_rotated_energy_gradient

        # sets the value of self._current_partial_unitary to U_1 for k=1 iteration
        current_partial_unitary = new_partial_unitary

        # sets the value of self._current_rotated_energy_gradient to gradf(U_1) for k=1 iteration
        current_rotated_energy_gradient = new_rotated_energy_gradient

        # updates iteration number to 1
        iteration_number += 1

        # calculates f(U_1) and stores it in self._P4_array[1].
        P4_array[1] = objfunc(partial_unitary=current_partial_unitary)
        if self._callback is not None:
            self._callback(iteration_number, P4_array[1].item())

        # calculate S_1 and store it in self._St_array[0]
        St_array[0] = (1 - self.decay_factor)*torch.abs(P4_array[1] - P4_array[2]) + self.decay_factor*St_array[1]

        # calculates U_2 (from U_1 and gradf(U_1)) and stores it in temporary variable
        new_partial_unitary = self.compute_updated_partial_unitary(iteration_number=iteration_number,
                                                                   current_partial_unitary=current_partial_unitary,
                                                                   previous_partial_unitary=previous_partial_unitary,
                                                                   current_rotated_energy_gradient=current_rotated_energy_gradient,
        ...
        # calculates gradf(U_1) and stores it in temporary variable
        new_rotated_energy_gradient = gradient_function(new_partial_unitary, objfunc)

        # copies U_1 into self._previous_partial_unitary for k=2 iteration
        previous_partial_unitary = current_partial_unitary

        # copies gradf(U_1) into self._previous_rotated_energy_gradient for k=2 iteration
        previous_rotated_energy_gradient = current_rotated_energy_gradient

        # override value of self._current_partial_unitary to make it U_2
        current_partial_unitary = new_partial_unitary

        # override value of self._current_rotated_energy_gradient to make it gradf(U_2)
        current_rotated_energy_gradient = new_rotated_energy_gradient

        # update iteration number to make it 2
        iteration_number += 1

        
        # calculates and stores f(U_2) in P4_array[0].
        P4_array[0] = objfunc(partial_unitary=current_partial_unitary)
        if self._callback is not None:
            self._callback(iteration_number, P4_array[0].item())


        # roll self._St_array so that S_1 is stored in self._St_array[1]
        St_array = np.roll(St_array, shift = 1) 

        # calculate S_2 and store it in self._St_array[0]
        St_array[0] = (1 - self.decay_factor)*torch.abs(P4_array[0] - P4_array[1]) + self.decay_factor*St_array[1]

        # calculates U_3 (from U_2 and gradf(U_2)) and stores it in temporary variable
        new_partial_unitary = self.compute_updated_partial_unitary(iteration_number=iteration_number,
                                                                   current_partial_unitary=current_partial_unitary,
                                                                   previous_partial_unitary=previous_partial_unitary,
                                                                   current_rotated_energy_gradient=current_rotated_energy_gradient,
        ...
        # calculates gradf(U_2) and stores it in temporary variable
        new_rotated_energy_gradient = gradient_function(new_partial_unitary, objfunc) 

        # copies U_2 into self._previous_partial_unitary for k=3 iteration
        previous_partial_unitary = current_partial_unitary

        # copies gradf(U_2) into self._previous_rotated_energy_gradient for k=3 iteration
        previous_rotated_energy_gradient = current_rotated_energy_gradient

        # override value of self._current_partial_unitary to make it U_3
        current_partial_unitary = new_partial_unitary

        # override value of self._current_rotated_energy_gradient to make it gradf(U_3)
        current_rotated_energy_gradient = new_rotated_energy_gradient

        # update iteration number to make it 3
        iteration_number += 1
        ...
        while St_array[0] > self.stopping_tolerance and iteration_number <= self.maxiter:
            ...
            # roll self._P4_array so that f(U_k-1) is stored in self._P4_array[1] and f(U_k-2) is stored in self._P4_array[2]
            P4_array = np.roll(P4_array, shift = 1)

            # set self._P4_array[0] to f(U_k)
            P4_array[0] = objfunc(partial_unitary=current_partial_unitary)
            ...
            # roll self._St_array so that S_k-1 is stored in self._St_array[0]
            St_array = np.roll(St_array, shift = 1)

            # set self._St_array[0] to S_k
            St_array[0] = (1 - self.decay_factor)*torch.abs(P4_array[1] - P4_array[2]) + self.decay_factor*St_array[1]

            # self._St_array should now be [S_k, S_k-1]
            # calculates U_k+1 (from U_k and gradf(U_k)) and stores it in temporary variable
            new_partial_unitary = self.compute_updated_partial_unitary(iteration_number=iteration_number,
                                                                       current_partial_unitary=current_partial_unitary,
                                                                       previous_partial_unitary=previous_partial_unitary,
                                                                       current_rotated_energy_gradient=current_rotated_energy_gradient,
            ...
            # calculates gradf(U_k+1) and stores it in temporary variable
            new_rotated_energy_gradient = gradient_function(new_partial_unitary, objfunc)

            # copies U_k into self._previous_partial_unitary for k=k+1 iteration
            previous_partial_unitary = current_partial_unitary

            # copies gradf(U_k) into self._previous_rotated_energy_gradient for k=k+1 iteration
            previous_rotated_energy_gradient = current_rotated_energy_gradient

            # override value of self._current_partial_unitary to make it U_K+1
            current_partial_unitary = new_partial_unitary

            # override value of self._current_rotated_energy_gradient to make it gradf(U_K+1)
            current_rotated_energy_gradient = new_rotated_energy_gradient

            # update iteration number to make it k+1
            iteration_number += 1
